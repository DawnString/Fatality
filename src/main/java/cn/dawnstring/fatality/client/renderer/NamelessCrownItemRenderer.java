...
@OnlyIn(Dist.CLIENT)
public class NamelessCrownItemRenderer
{
    ...
    public static void onRenderPlayer(RenderPlayerEvent.Pre event) {
        ...
        if (hasNamelessCrownEquipped(player)) {
            renderNamelessCrownHalo(player, event.getPoseStack(),
                    event.getMultiBufferSource(), event.getPackedLight(),
                    event.getPartialTick());
        }
    }

    private static void renderNamelessCrownHalo(Player player, PoseStack poseStack,
    ...
    private static void renderCrownHaloModel(PoseStack poseStack, MultiBufferSource bufferSource,
        ...
        // 获取无名王冠的物品实例
        var namelessCrownItem = ModItems.NAMELESS_CROWN.get();
        ItemStack crownStack = new ItemStack(namelessCrownItem);
    ...
    private static boolean hasNamelessCrownEquipped(Player player) {
        return cn.dawnstring.fatality.items.accessory.NamelessCrown.isPlayerWearingCrown(player);
    }
}