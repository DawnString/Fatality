...
public class SpiritualFireDamageSource extends DamageSource {

    public SpiritualFireDamageSource(Holder<DamageType> damageType, @Nullable Entity directEntity, @Nullable Entity causingEntity, @Nullable Vec3 damageSourcePosition) {
        super(damageType, directEntity, causingEntity, damageSourcePosition);
    }

    // 静态工厂方法，方便创建实例
    public static SpiritualFireDamageSource spiritualFire(@Nullable Entity directEntity, @Nullable Entity causingEntity) {
        // 使用魔法伤害类型作为基础
        Holder<DamageType> damageType = getMagicDamageTypeHolder(directEntity);

        return new SpiritualFireDamageSource(damageType, directEntity, causingEntity, null);
    }

    public static SpiritualFireDamageSource spiritualFire() {
        return spiritualFire(null, null);
    }
    ...
    public Component getLocalizedDeathMessage(LivingEntity entity) {
        String messageKey = "death.attack.spiritual_fire";
