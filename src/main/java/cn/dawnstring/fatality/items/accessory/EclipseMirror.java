...
public class EclipseMirror extends AccessoryItem
{
    ...
    // 减伤比例
    private static final float DAMAGE_REDUCTION_PERCENTAGE = 0.60f; // 60%减伤
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        if (event.getEntity() instanceof Player player) {
            // 检查玩家是否佩戴日食之镜
            if (hasEclipseMirrorEquipped(player)) {
    ...
    private static void spawnDamageReductionParticles(Player player) {
        if (player.level().isClientSide()) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                player.level().addParticle(net.minecraft.core.particles.ParticleTypes.SQUID_INK,
                        x, y, z, 0.0, 0.04, 0.0);
            }
        }
    }
    ...
    // 检查玩家是否佩戴日食之镜
    private static boolean hasEclipseMirrorEquipped(Player player) {
        // 检查饰品栏
        ...
        if (accessoryInventory != null) {
            for (int i = 0; i < accessoryInventory.getItemHandler().getSlots(); i++) {
                ...
                if (accessory.getItem() instanceof EclipseMirror) {
                    return true;
                }
            }
        }
