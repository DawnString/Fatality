...
public class ObsidianBoots extends AccessoryItem {
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        if (event.getEntity() instanceof Player player) {
            // 检查玩家是否装备了黑曜石靴
            boolean hasObsidianBoots = hasObsidianBootsEquipped(player);  // 修复：使用正确的检测方法
    ...
    private static boolean isFireDamage(DamageSource damageSource) {
        // 使用DamageSource的msgId或类型来判断伤害类型
        String damageType = damageSource.getMsgId();

        // 检查是否为火焰相关伤害
        return damageType.contains("fire") ||
                damageType.contains("lava") ||
                damageType.contains("burn") ||
                damageType.contains("hot_floor") ||
                damageType.contains("inFire") ||
                damageType.contains("onFire");
    }
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            // 检查玩家是否装备了黑曜石靴（修复错误的检测方法）
            boolean hasObsidianBoots = hasObsidianBootsEquipped(player);
    ...
    // 检查玩家是否装备了黑曜石靴（修复方法名和类型）
    public static boolean hasObsidianBootsEquipped(Player player) {
        // 检查饰品栏
        ...
        if (accessoryInventory != null) {
            for (int i = 0; i < accessoryInventory.getItemHandler().getSlots(); i++) {
                ...
                if (stack.getItem() instanceof ObsidianBoots) {  // 修复：应该检查 ObsidianBoots 而不是 LavaBoots
                    return true;
                }
            }
        }
