...
public class JetBoots extends AccessoryItem {

    // 飞行状态跟踪
    private static final Map<UUID, JetBootsFlightData> flightDataMap = new HashMap<>();
    ...
    public void applyEffects(Player player, ItemStack stack) {
        ...
        if (!flightDataMap.containsKey(playerId)) {
            flightDataMap.put(playerId, new JetBootsFlightData(maxFlightTime, maxFlightSpeed, verticalAcceleration));
        }
    }
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            // 检查玩家是否装备了喷气靴
            boolean hasJetBoots = hasJetBootsEquipped(player);
            ...
            // 获取飞行数据（如果不存在则创建）
            JetBootsFlightData flightData = flightDataMap.get(playerId);
            if (flightData == null) {
                flightData = new JetBootsFlightData(10.0f, 1.5f, 0.15f);
                flightDataMap.put(playerId, flightData);
            }

            handleJetBootsFlight(player, flightData);
        }
    }

    // 处理喷气靴飞行逻辑
    private static void handleJetBootsFlight(Player player, JetBootsFlightData flightData) {
    ...
    // 处理向上飞行
    private static void handleUpwardFlight(Player player, JetBootsFlightData flightData) {
    ...
    // 检查玩家是否装备了喷气靴
    public static boolean hasJetBootsEquipped(Player player) {
        // 检查饰品栏
        ...
        if (accessoryInventory != null) {
            for (int i = 0; i < accessoryInventory.getItemHandler().getSlots(); i++) {
                ...
                if (stack.getItem() instanceof JetBoots) {
                    return true;
                }
            }
        }
    ...
    // 喷气靴飞行数据内部类
    public static class JetBootsFlightData {
        ...
        public JetBootsFlightData(float maxFlightTime, float maxFlightSpeed, float verticalAcceleration) {
