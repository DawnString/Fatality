...
public class MechanicalHeart extends AccessoryItem {
    ...
    private static final float DAMAGE_BONUS = 0.50f; // 50%伤害加成

    // 属性修改器UUID
    private static final UUID DAMAGE_BONUS_UUID = UUID.fromString("12345678-1234-1234-1234-123456789abc");

    public MechanicalHeart(Properties properties) {
        super(properties);
     ...
     * 检查玩家是否佩戴机械之心
     */
    public static boolean hasMechanicalHeartEquipped(Player player) {
        ...
        if (accessoryInventory != null) {
            for (int i = 0; i < accessoryInventory.getItemHandler().getSlots(); i++) {
                ...
                if (accessory.getItem() instanceof MechanicalHeart) {
                    return true;
                }
            }
        }
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            if (hasMechanicalHeartEquipped(player)) {
                UUID playerId = player.getUUID();
