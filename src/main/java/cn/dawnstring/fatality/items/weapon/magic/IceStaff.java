...
public class IceStaff extends BaseWeapon
{
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (target != null) {
            ...
            if (!level.isClientSide()) {
                ...
                // 显示目标信息
                player.displayClientMessage(net.minecraft.network.chat.Component.literal("§9对 " + target.getName().getString() + " 施放了冰龙卷！"), true);
            } else {
                // 在客户端也播放音效和显示信息
                level.playSound(null, player.getX(), player.getY(), player.getZ(),
                        SoundEvents.GLASS_BREAK, SoundSource.PLAYERS, 1.0F, 0.8F);
