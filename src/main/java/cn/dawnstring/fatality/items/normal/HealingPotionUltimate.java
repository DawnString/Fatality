 ...
 */
public class HealingPotionUltimate extends NormalItem {
    private static final int HEAL_AMOUNT = 400; // 恢复400点生命值
    private static final int TREATMENT_SATURATION_DURATION = 1200; // 60秒 * 20tick/秒 = 1200tick

    public HealingPotionUltimate() {
        super();
        this.setStory("终极治疗药水，可以立即恢复400点生命值，但会暂时降低生命恢复效果。");
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            ...
            player.displayClientMessage(Component.literal("§a使用终极治疗药水！恢复400点生命值，获得治疗饱和效果（60秒）"), true);
