 ...
 */
public class ManaPotionBasic extends NormalItem {
    private static final int MANA_RESTORE_AMOUNT = 100; // 恢复100点魔法值
    private static final int MAGIC_FADE_DURATION = 300; // 15秒 * 20tick/秒 = 300tick

    public ManaPotionBasic() {
        super();
        this.setStory("初级魔力药水，可以立即恢复100点魔法值，但会暂时降低魔法伤害和恢复效果。");
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            ...
            player.displayClientMessage(Component.literal("§a使用初级魔力药水！恢复100点魔法值，获得魔力衰减效果（15秒）"), true);
