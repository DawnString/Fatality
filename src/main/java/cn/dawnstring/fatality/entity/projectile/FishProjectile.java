 ...
 * 特性：竖直落下，造成伤害
 */
public class FishProjectile extends AbstractArrow
{
    private static final EntityDataAccessor<Float> DATA_DAMAGE = SynchedEntityData.defineId(FishProjectile.class, EntityDataSerializers.FLOAT);

    private int lifeTime = 0;
    private final int maxLifeTime = 80; // 4秒生命周期（20tick/秒）
    private ItemStack weaponItem; // 存储发射的武器

    public FishProjectile(EntityType<? extends FishProjectile> type, Level level) {
        super(type, level);
        this.setNoGravity(false); // 鱼受重力影响
        this.entityData.set(DATA_DAMAGE, 250.0f); // 默认伤害
    }

    public FishProjectile(Level level, LivingEntity shooter, ItemStack weapon, float damage) {
        this(ModEntities.FISH_PROJECTILE.get(), level);
    ...
    protected void onHit(HitResult result) {
        ...
        if (!this.level().isClientSide()) {
            ...
            this.level().playSound(null, this.getX(), this.getY(), this.getZ(),
                    SoundEvents.SALMON_FLOP, SoundSource.BLOCKS, 0.8F, 1.2F);
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("FishDamage")) {
            this.entityData.set(DATA_DAMAGE, compound.getFloat("FishDamage"));
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        compound.putFloat("FishDamage", getFishDamage());
    }
}